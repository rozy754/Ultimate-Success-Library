            ...
            <div className="space-y-4">
              {peakHours.map((hour, index) => (
                <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    <p className="text-sm text-muted-foreground">At {hour.time}</p>
                    <div className="text-lg font-semibold">{hour.occupancy}% Occupied</div>
                  </div>
                  <Progress value={hour.occupancy} className="w-1/2 h-2" />
                </div>
              ))}
            </div>
          </CardContent>
